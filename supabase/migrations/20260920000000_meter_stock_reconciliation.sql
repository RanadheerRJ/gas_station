-- Authoritative meter -> book stock -> physical stock reconciliation.
-- Physical dips are observations and never rewrite tanks.current_stock (book stock).
...
create index stock_movements_tank_time_idx on public.stock_movements(tank_id, recorded_at desc);
create index stock_movements_station_time_idx on public.stock_movements(station_id, recorded_at desc);
create unique index stock_movements_reference_line_uidx
...
create unique index tank_readings_request_uidx on public.tank_readings(recorded_by, request_id)
